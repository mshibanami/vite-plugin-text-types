...
export function generateTsContent(
  ...
  for (const { key, content } of entries) {
    if (content.length > maxLiteralLength) {
      lines.push(`  ${JSON.stringify(key)}: ${JSON.stringify(content)} as string,`);
    } else {
      lines.push(`  ${JSON.stringify(key)}: ${JSON.stringify(content)},`);
    }
  }
  ...
  lines.push(`    : [vars: { [P in ExtractVars<(typeof texts)[K]>]: string | number }]`);
  lines.push(`): string {`);
  lines.push(`  const t: string = texts[id];`);
  lines.push(`  const vars = args[0];`);
  lines.push(`  if (!vars) return t;`);
  ...
  lines.push(`    const val = (vars as any)[key.trim()];`);
  lines.push(
    `    return val !== undefined ? String(val) : ${JSON.stringify(delimiters[0])} + key + ${JSON.stringify(delimiters[1])};`,
  );
  lines.push(`  });`);
  lines.push(`}`);
